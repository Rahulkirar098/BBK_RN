...
function reducer(state: any, action: any) {
    switch (action.type) {
        ...
        case "CREATE_BOAT":
            return { ...state, editingBoat: action.payload, tab: "boats", modalOpen: true };

        case "CREATE_CAPTAIN":
            return { ...state, editingCaptain: action.payload, tab: "captains", modalOpen: true };
        ...
        case "SET_FIELD":
            ...
            return { ...state, [action.target]: { ...target, [action.field]: action.value } };
...
const OperatorProfile = () => {
    ...
    useEffect(() => {
        ...
        const unsubBoats = onSnapshot(collection(db, "users", uid, "boats"), (snap) => {
            const boats = snap.docs.map((d: any) => ({ id: d.id, ...d.data() } as Boat));
            dispatch({ type: "SET_BOATS", payload: boats });
        });

        const unsubCaptains = onSnapshot(collection(db, "users", uid, "captains"), (snap) => {
            const captains = snap.docs.map((d: any) => ({ id: d.id, ...d.data() } as Captain));
    ...
    const saveCaptain = async () => {
        ...
        const id = state.editingCaptain.id || Math.random().toString(36).substr(2, 9);
            ...
            <View style={styles.container}>
                ...
                <View style={styles.row}>
                    ...
                    <StatCard
                        ...
                        label="Captains"
                        onPress={() => dispatch({ type: "OPEN_MODAL", payload: "captains" })}
                    />
                </View>
                            ...
                            <ScrollView nestedScrollEnabled={true}>
                                            ...
                                            <View style={styles.itemRow}>
                                                ...
                                                <View style={styles.iconRow}>
                                                    <TouchableOpacity
                                                        onPress={() =>
                                                            dispatch({
                                                                type: state.tab === "boats" ? "EDIT_BOAT" : "EDIT_CAPTAIN",
                                                                payload: item,
                                                            })
                                                        }
                                                    >
                                                    ...
                                                    <TouchableOpacity onPress={() => deleteItem(state.tab, item.id)}>
                                                        <Trash2 size={18} />
                                                    </TouchableOpacity>
                                                </View>
                                            </View>
                                ...
                                {!state.editingBoat && !state.editingCaptain && (
                                    <Button
                                        ...
                                        onPress={() =>
                                            dispatch({
                                                type: state.tab === "boats" ? "CREATE_BOAT" : "CREATE_CAPTAIN",
                                                payload: state.tab === "boats" ? { ...emptyBoat } : { ...emptyCaptain },
                                            })
                                        }
                                    />
                                )}
                                ...
                                {state.editingBoat && (
                                    <View style={{ gap: horizontalScale(5) }}>
                                        <Input
                                            ...
                                            onChangeText={(v) =>
                                                dispatch({ type: "SET_FIELD", target: "editingBoat", field: "boatName", value: v })
                                            }
                                        />
                                        <Input
                                            ...
                                            onChangeText={(v) =>
                                                dispatch({ type: "SET_FIELD", target: "editingBoat", field: "boatCompany", value: v })
                                            }
                                        />
                                        <Input
                                            ...
                                            onChangeText={(v) =>
                                                dispatch({ type: "SET_FIELD", target: "editingBoat", field: "boatModel", value: v })
                                            }
                                        />
                                        <Input
                                            ...
                                            onChangeText={(v) =>
                                                dispatch({ type: "SET_FIELD", target: "editingBoat", field: "boatCapacity", value: Number(v) })
                                            }
                                        />
                                        ...
                                        <Select
                                            ...
                                            onChange={(v) =>
                                                dispatch({ type: "SET_FIELD", target: "editingBoat", field: "status", value: v })
                                            }
                                        ...
                                        <Button label="Pick Image" onPress={() => pickImage("boatImage")} />
                                ...
                                {state.editingCaptain && (
                                    <View style={{ gap: horizontalScale(5) }}>
                                        <Input
                                            ...
                                            onChangeText={(v) =>
                                                dispatch({ type: "SET_FIELD", target: "editingCaptain", field: "name", value: v })
                                            }
                                        />
                                        ...
                                        <Select
                                            ...
                                            options={statusOptions}
                                            onChange={(v) =>
                                                dispatch({ type: "SET_FIELD", target: "editingCaptain", field: "status", value: v })
                                            }
                                        ...
                                        <Select
                                            ...
                                            options={language}
                                            onChange={(v) =>
                                                dispatch({ type: "SET_FIELD", target: "editingCaptain", field: "language", value: v })
                                            }
                                        ...
                                        <Button label="Pick Image" onPress={() => pickImage("captainImage")} />
...
const styles = StyleSheet.create({
    ...
    logoutText: { color: "red", fontSize: 18, fontWeight: "600" },
